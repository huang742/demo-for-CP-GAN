...
class EncoderCNN(nn.Module):
    def __init__(self, embed_size):
        ...
        self.linear = nn.Linear(f(256) * f(256) * 2048, embed_size)
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, features, captions, lengths):
        ...
        Softoutput = self.softmax(outputs)
        Loutputs = self.logsoftmax(outputs)
        # Loutputs = self.softmax(outputs)

        return outputs, Loutputs,Softoutput
    ...
    def step(self, x, states):
        ...
        return outputs, hiddens, states


    def init_step(self, features, states=None):
    ...
    def sample(self, batch_size, seq_len, x, features, states=None):
        features = features.unsqueeze(1)
        ...
        emb = self.embed(lis[0])
        hiddens, states = self.lstm(emb, states)
        samples.append(lis[0].squeeze(1))

        # output, hiddens, states = self.init_step(features)

        for i in range(given_len-1):
            emb = self.embed(lis[i+1])
            hiddens, states = self.lstm(emb, states)
            samples.append(lis[i+1].squeeze(1))
        output = self.linear(hiddens.squeeze(1))
        pre = self.softmax(output).multinomial(1)
        pre = pre.squeeze(1)
        samples.append(pre)
    ...
    def gen_sample(self, features, lengths,states=None, sampel_max = False):
        ...
        for i in range(length):
            ...
            if sampel_max:
                ...
                predicted = self.softmax(outputs).multinomial(1)
                prob = self.softmax(outputs).gather(1, predicted)
                predicted = predicted.squeeze()
            # prob = outputs[pre]

            # predicted = outputs.multinomial(1)
            sampled_ids.append(predicted)
            sampled_prob.append(prob)
            # sampled_pre.append(pre)
            inputs = self.embed(predicted)                       # inputs: (batch_size, embed_size)
            inputs = inputs.unsqueeze(1)                         # inputs: (batch_size, 1, embed_size)
        sampled_ids = torch.stack(sampled_ids, 1)                # sampled_ids: (batch_size, max_seq_length)
        sampled_prob = torch.stack(sampled_prob, 1)
        # sampled_pre = torch.stack(sampled_pre, 1)
        return sampled_ids, sampled_prob

    def gen_pred(self, features, lengths,states=None):
        """Generate captions for given image features using greedy search."""
        sampled_ids = []
        preds = []
        inputs = features.unsqueeze(1)
